...
async def test_auth_with_bad_token():
    ...
    assert connected
    await communicator.send_json_to(data={"cmd": "auth", "params": "bad-auth-token"})
...
async def test_auth_with_valid_token(admin_user):
    ...
    connected, subprotocol = await communicator.connect()
    assert connected
    await communicator.send_json_to(data={"cmd": "auth", "params": admin_user.auth_token.key})
    response = await communicator.receive_json_from()
...
async def test_sub_private_topics_without_auth():
    ...
    connected, subprotocol = await communicator.connect()
    assert connected
    await communicator.send_json_to(data={"cmd": "sub", "params": ["robot#1.log"]})
    response = await communicator.receive_json_from()
